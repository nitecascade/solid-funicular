...
def generate_categories(api_key):
    offset = 0
    while True:
        ...
        r = requests.get('https://api.meetup.com/2/categories', params=payload)
        json = r.json()
...
def go(api_key):
    ...
    all_categories = generate_categories(api_key)
    for category in all_categories:
        jdata = json.dumps(category)
        print("{}".format(jdata))
